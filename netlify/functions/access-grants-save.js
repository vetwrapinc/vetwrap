const { requireAccess, json, ACCESS_ROLES, getSupabaseConfig, hashPassword } = require('./_auth')
...
exports.handler = async (event) => {
  ...
  const allowPassword = Boolean(payload.allowPassword)
  const hasPassword = Boolean(payload.hasPassword)
  const password = sanitize(payload.password, 200)
  if (allowPassword && !password && !hasPassword && !id) {
    return json(400, { error: 'Set a password to enable email sign-in.' })
  }
  ...
  if (!allowPassword) {
    record.password_salt = null
    record.password_hash = null
    record.password_updated_at = null
  } else if (password) {
    const hashed = hashPassword(password)
    record.password_salt = hashed.salt
    record.password_hash = hashed.hash
