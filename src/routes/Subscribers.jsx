...
export default function Subscribers() {
  ...
  const [passwordLoginEmail, setPasswordLoginEmail] = React.useState('')
  const [passwordLoginPassword, setPasswordLoginPassword] = React.useState('')
  const [passwordLoginMessage, setPasswordLoginMessage] = React.useState(null)
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('vetwraps-admin-token-changed', syncAdminTokenState)
    window.addEventListener('storage', syncAdminTokenState)
    return () => {
      window.removeEventListener('vetwraps-admin-token-changed', syncAdminTokenState)
  ...
  function handleUseToken() {
    ...
    try {
      window.dispatchEvent(new Event('vetwraps-admin-token-changed'))
    } catch {}
  ...
  async function handlePasswordLogin(e) {
    ...
    if (!email || !password) {
      setPasswordLoginMessage({ type: 'error', text: 'Enter your email and password.' })
      return
    }
    ...
    try {
      ...
      localStorage.setItem('vetwraps_admin_token', data.token)
      try {
        window.dispatchEvent(new Event('vetwraps-admin-token-changed'))
      } catch {}
      setStoredAdminToken(data.token)
      setAdminTokenInput(data.token)
      setPasswordLoginMessage({ type: 'success', text: 'Signed in successfully.' })
      setPasswordLoginPassword('')
      setQuotesError(null)
      setEmployeeError(null)
      setAccessMessage(null)
      refreshAccess()
      if (view === 'clients' || view === 'access') {
        setView('employees')
      }
    } catch (err) {
      setPasswordLoginMessage({ type: 'error', text: err.message || 'Incorrect email or password.' })
    } finally {
  ...
  function handleAccessCheckboxChange(e) {
    const { name, checked } = e.target
    setAccessForm((prev) => ({
      ...prev,
      [name]: checked,
      ...(name === 'allowPassword' && !checked ? { password: '', confirmPassword: '', hasPassword: false } : {})
    }))
  }
  ...
  function startEditingAccess(grant) {
    ...
    setAccessForm({
      ...
      notes: grant.notes || '',
      allowPassword: Boolean(grant.allowPassword),
      password: '',
      confirmPassword: '',
      hasPassword: Boolean(grant.allowPassword && grant.passwordUpdatedAt)
    })
  }
  ...
  async function handleSaveAccess(e) {
    ...
    const wantsPassword = Boolean(accessForm.allowPassword)
    const newPassword = (accessForm.password || '').trim()
    const confirmPassword = (accessForm.confirmPassword || '').trim()
    const hasExistingPassword = Boolean(accessForm.hasPassword)
    if (newPassword && newPassword !== confirmPassword) {
      setAccessMessage({ type: 'error', text: 'Password confirmation does not match.' })
      return
    }
    if (wantsPassword && !newPassword && !hasExistingPassword && !accessForm.id) {
      setAccessMessage({ type: 'error', text: 'Provide a password to enable email sign-in.' })
      return
    }
          ...
          <div className="text-center">
            ...
            <div className="mt-4 max-w-sm mx-auto space-y-3">
              ...
              <div className="pt-4 border-t border-white/10">
                <h2 className="text-sm font-semibold text-white/80 mb-2">Email Sign-In</h2>
                <form onSubmit={handlePasswordLogin} className="space-y-3">
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2 rounded bg-white/10 border border-white/15 hover:border-accent-blue/50 disabled:opacity-60"
                  >
                    {passwordLoginLoading ? 'Signing In…' : 'Sign In with Email'}
                  </button>
                </form>
                {passwordLoginMessage && (
                  <p
                    className={`mt-2 text-xs ${
                      passwordLoginMessage.type === 'error' ? 'text-red-400' : 'text-accent-blue'
                    }`}
                  >
            ...
            {view === 'employees' && canViewEmployees ? (
              ...
              <section className="mt-8 space-y-6">
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
                  ...
                  <StatCard label="Email Sign-In" value={accessStats.passwordEnabled} description="Passwords enabled" />
                    ...
                    <div className="md:col-span-2">
                      <label className="flex items-center gap-2 text-sm">
                        <input
                          type="checkbox"
                          name="allowPassword"
                          checked={accessForm.allowPassword}
                          onChange={handleAccessCheckboxChange}
                          className="w-4 h-4 rounded border-white/20 bg-white/10"
                        />
                        <span>Allow email/password sign-in</span>
                      </label>
                      <p className="text-xs text-white/50 mt-1">
                        {accessForm.allowPassword
                          ? 'Set a password below. Leave blank to keep the current password.'
                          : 'Disable this to require Google sign-in only.'}
                      </p>
                    </div>
                    {accessForm.allowPassword && (
                      <>
                        <div>
                          <label htmlFor="accessPassword" className="block text-sm mb-1">
                            {accessForm.id ? 'New Password' : 'Password'}
                          </label>
                          <input
                            id="accessPassword"
                            name="password"
                            ...
                            onChange={handleAccessInputChange}
                            placeholder={accessForm.id ? 'Leave blank to keep existing password' : 'Minimum 12 characters'}
                            className="w-full bg-white/5 border-white/15 rounded-md px-3 py-2"
                            minLength={12}
                          />
                        </div>
                        <div>
                          <label htmlFor="accessPasswordConfirm" className="block text-sm mb-1">
                            Confirm Password
                          </label>
                          <input
                          ...
                          {filteredAccessGrants.map((grant) => (
                            <tr key={grant.id || grant.email} className="border-t border-white/10">
                              ...
                              <td className="py-2 pr-4 text-white/70 text-xs">
                                {grant.allowPassword ? 'Email & Google' : 'Google only'}
                                {grant.allowPassword && grant.passwordUpdatedAt && (
                                  <span className="block text-white/40">
                                    Updated {formatDate(grant.passwordUpdatedAt)}
                                  </span>
                                )}
                              </td>
...
function mapAccessGrant(row = {}) {
  return {
    ...
    allowPassword: Boolean(row.allow_password || row.allowPassword),
    passwordUpdatedAt: row.password_updated_at || row.passwordUpdatedAt || null
  }
}
...
function computeAccessStats(items = []) {
  ...
  items.forEach((item) => {
    ...
    if (item.allowPassword) passwordEnabled += 1
