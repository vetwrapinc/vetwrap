...
export default function Navbar() {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('vetwraps-admin-token-changed', syncToken)
    return () => {
      window.removeEventListener('storage', syncToken)
      window.removeEventListener('focus', syncToken)
      window.removeEventListener('vetwraps-admin-token-changed', syncToken)
    }
  }, [])

  const showDashboardLink = hasDashboardAccess || hasAdminToken

  const handleAdminTokenLogout = React.useCallback(() => {
    try {
  ...
  const handleLogoutAll = React.useCallback(() => {
    handleAdminTokenLogout()
    logout()
  }, [handleAdminTokenLogout, logout])
            ...
            <>
              ...
              {user ? (
                ...
                hasAdminToken && (
                  <button
                    type="button"
                    onClick={handleAdminTokenLogout}
                    className="text-[11px] tracking-[0.2em] uppercase text-white/60 hover:text-white focusable transition-colors"
                  >
                    Log Out
                  </button>
